...
def extract_kart_objects(
    ...
    for detection in detections:
        ...
        # only consider karts (class_id == 1)
        if class_id != 1:
            continue
        ...
        # calculate center point of the kart
        center_x = (x1_scaled + x2_scaled) // 2
        center_y = (y1_scaled + y2_scaled) // 2

        # add kart object to the list
        kart_objects.append({
            "instance_id": track_id,
            "kart_name": kart_names[track_id] if track_id < len(kart_names) else f"Unknown Kart {track_id}",
    ...
    # determine the center kart (the one closest to the image center)
...
def generate_qa_pairs(info_path: str, view_index: int, img_width: int = 150, img_height: int = 100) -> list:
    ...
    # Extract kart objects and track information
    ...
    # total karts question
    qa_pairs.append({
        "question": "How many karts are there in the scenario?",
        "answer": str(len(kart_objects))
    })

    # track information question
    qa_pairs.append({
    ...
    # counting questions
    left_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][0] < ego_kart["center"][0])
    right_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][0] > ego_kart["center"][0])
    front_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][1] < ego_kart["center"][1])
    behind_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][1] > ego_kart["center"][1])
...
"""
def generate_dataset(data_dir: str = "data/train", output_name: str = "generated_qa_pairs.json"):
    """
    ...
    for info_file in info_files:
        ...
        for view_index in range(num_views):
            ...
            try:
                ...
                for pair in pairs:
                    pair["image_file"] = f'train/{img_name}'
    ...
    print(f"Successfully saved {len(all_qa_pairs)} QA pairs to {output_path}")
